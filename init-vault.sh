...
vault kv put ayofootball/config \
  DATABASE_URL="host=pgsql-server user=root password=admin dbname=ayofootball port=5432 sslmode=disable TimeZone=Asia/Jakarta" \
  TEAM_SERVICE_PORT="8002" \
  TEAM_SERVICE_URL="team-srv" \
  AUTH_SERVICE_PORT="8001" \
  AUTH_SERVICE_URL="auth-srv" \
  GATEWAY_PORT="8000" \
  PLAYER_SERVICE_PORT="8003" \
  PLAYER_SERVICE_URL="player-srv" \
  MATCH_SERVICE_PORT="8004" \
  MATCH_SERVICE_URL="match-srv" \
